...
class ConfigBeanDefinitionParser implements BeanDefinitionParser {
	...
	public BeanDefinition parse(Element element, ParserContext parserContext) {
		...
		// 解析aop:config子节点下的aop:pointcut/aop:aspect
	...
	/**
	 * 配置支持aop:config标签创建的BeanDefinition的自动代理创建器
	 *
	...
	private void parseAdvisor(Element advisorElement, ParserContext parserContext) {
		// 解析<aop:advisor>节点，最终创建beanClass为`DefaultBeanFactoryPointcutAdvisor`
	...
	private void parseAspect(Element aspectElement, ParserContext parserContext) {
		...
		try {
			...
			// 采用的时DeclareParentsAdvisor作为beanClass加载
	...
	private AbstractBeanDefinition createAdviceDefinition(
		...
		// 解析节点是否含有`returning`/`throwing`/`args-names`。有则设置
