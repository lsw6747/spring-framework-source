...
public class DefaultAdvisorChainFactory implements AdvisorChainFactory, Serializable {
	...
	public List<Object> getInterceptorsAndDynamicInterceptionAdvice(
		...
		// 这里调用了一个单例模式，获取DefaultAdvisorAdapterRegistry实例
		// 在Spring中把每一个功能都分的很细，每个功能都会有相应的类取处理，符合单一职责原则的地方很多，这也是值得我们借鉴的一个地方
		// AdvisorAdapterRegistry这个类的主要作用是将Advice适配为Advisor，将Advisor适配为对应的MethodInterceptor
		AdvisorAdapterRegistry registry = GlobalAdvisorAdapterRegistry.getInstance();
		Advisor[] advisors = config.getAdvisors();
		// 创建一个初始大小为之前获取到的通知个数的集合
		...
		for (Advisor advisor : advisors) {
			...
			if (advisor instanceof PointcutAdvisor) {
				...
				// 如果提前进行过切点的匹配或者当前的Advisor使用于目标类
