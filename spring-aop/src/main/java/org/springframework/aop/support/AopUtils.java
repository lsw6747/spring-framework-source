...
public abstract class AopUtils {
	...
	public static boolean canApply(Pointcut pc, Class<?> targetClass, boolean hasIntroductions) {
		...
		// 进行切入点表达式的匹配最重要的就是ClassFilter和MethodMatcher这两个方法的实现。
		// MethodMatcher中有两个mathces方法。一个参数是只有Method对象和targetClass，另一个参数有Method对象和targetClass对象还有一个Method的方法参数。
		// 他们两个的区别是：两个参数的mathces是用于静态的方法匹配，三个参数的matches是在运行期间动态的进行方法匹配的
		// 先进行ClassFilter的mathces方法校验
		// 首先这个类要在所匹配的规则下
		if (!pc.getClassFilter().matches(targetClass)) {
			return false;
		}

		// 在进行MethodMatcher方法级别的校验
		...
		for (Class<?> clazz : classes) {
			...
			for (Method method : methods) {
				...
				// 但是也可能有一些方法是不匹配切入点规则的，这里检测的是只有一个Method满足切点规则就返回true了
	...
	public static boolean canApply(Advisor advisor, Class<?> targetClass, boolean hasIntroductions) {
		// 如果这是IntroductionAdvisor的话，则调用IntroductionAdvisor类型的实例进行类的过滤
		// 这里是直接调用ClassFilter的mathces方法
	...
	public static List<Advisor> findAdvisorsThatCanApply(List<Advisor> candidateAdvisors, Class<?> clazz) {
		...
		for (Advisor candidate : candidateAdvisors) {
			// 判断我们的增强器对象是否实现了IntroductionAdvisor(很明显我们事务的没有实现，所以不会走下面的逻辑)
